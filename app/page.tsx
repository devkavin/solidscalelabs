...
const faqs = [
  {
    q: "How fast can this futuristic experience ship?",
    a: "Core direction can launch in 2-3 weeks, then we iterate in weekly motion and conversion sprints.",
  },
  {
    q: "Will performance drop with this much interaction?",
    a: "No. Animations are GPU-friendly transforms with selective motion, lazy sections, and strict performance budgets.",
  },
  {
    q: "Can our team edit this without breaking visuals?",
    a: "Yes. The system is tokenized and componentized so content updates remain safe while preserving design choreography.",
  },
  {
    q: "Is accessibility still covered in this aesthetic?",
    a: "Absolutely. Focus states, motion-reduction support, contrast checks, and keyboard access are first-class constraints.",
  },
];
    ...
    <>
      ...
      <section id="faq" className="py-16 md:py-24">
        <div className="container max-w-5xl">
          <span className="eyebrow">Future FAQ</span>
          <h2 className="mt-4 text-3xl font-semibold text-white md:text-5xl">Answers from the interface of tomorrow.</h2>
          <div className="mt-8 grid gap-3">
            {faqs.map((item, idx) => {
              const isOpen = idx === openFaq;
              return (
                <motion.div key={item.q} layout className={cn("faq-tile", isOpen && "faq-tile--active")}>
                  <button
                    className="focus-ring flex w-full items-center justify-between gap-4 px-5 py-4 text-left"
                    onClick={() => setOpenFaq(isOpen ? -1 : idx)}
                  >
                    <span className="font-medium text-white">{item.q}</span>
                    <span className="faq-badge">{isOpen ? "LIVE" : "OPEN"}</span>
                  </button>
                  <div className={cn("grid transition-all duration-300", isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]")}>
                    <p className="overflow-hidden px-5 pb-5 text-slate-300">{item.a}</p>
                  </div>
                </motion.div>
              );
            })}
